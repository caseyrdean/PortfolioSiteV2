      ...
      <section className="bg-gray-100 max-w-[54.6rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[26rem] hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-1 sm:mt-auto">
            {tags.map((tag, index) => (
              <li
                className="bg-black/[0.7] px-3 py-1 text-[0.8rem] uppercase tracking-wider text-white rounded-full dark:text-white/70"
                key={index}
              >
                {tag}
              </li>
        ...
        <Image
          ...
          quality={95}
          className="absolute hidden sm:block top-8 -right-40 w-[36.7rem] rounded-t-lg shadow-2xl
        transition 
